...
public class Dashboard {
    ...
    @FXML
    private PieChart medicamentStockPieChart;
    @FXML
    private BarChart<String, Number> pharmacyTopHospitalsBarChart;
    @FXML
    private BarChart<String, Number> medicamentCategoryBarChart;
    @FXML
    private LineChart<String, Number> medicamentExpiryLineChart;
    ...
    @FXML private Label eventStatsSummaryLabel;
    @FXML private PieChart eventThemePieChart;
    @FXML private BarChart<String, Number> eventMonthBarChart;
    @FXML private BarChart<String, Number> eventLieuBarChart;

    @FXML private PieChart userEngagementPie;
    ...
    public void initialize() {
        ...
        RechUserText.textProperty().addListener((obs, oldVal, newVal) -> {
            RechUser(null);
        });

        RechCreneau.setOnAction(e -> RechCreneau(null));

        RechEventText.textProperty().addListener((obs, oldVal, newVal) -> {
            RechEvent(null);
        });

        RechCreneau.setItems(FXCollections.observableArrayList(
                "Dispo",
                "Reserved"
        ));
        triUser.setItems(FXCollections.observableArrayList(
                "Nom A→Z", "Nom Z→A",
                "Prenom A→Z", "Prenom Z→A",
                "Email A→Z", "Email Z→A",
                "Role A→Z", "Role Z→A"
        ));

        triCreneau.setItems(FXCollections.observableArrayList(
                "Date ↑", "Date ↓",
                "Heure debut ↑", "Heure debut ↓",
                "Heure fin ↑", "Heure fin ↓"
        ));

        triEvent.setItems(FXCollections.observableArrayList(
                "Titre A→Z", "Titre Z→A",
                "Theme A→Z", "Theme Z→A",
                "Date ↑", "Date ↓",
                "Heure debut ↑", "Heure debut ↓",
        ...
        triUser.setOnAction(e -> trierUsers());
        triCreneau.setOnAction(e -> trierCreneaux());
        triEvent.setOnAction(e -> trierEvents());
    }
    ...
    private void initAnnoncesAdmin() {
        if (annonceAdminSort != null) {
            annonceAdminSort.setItems(FXCollections.observableArrayList(
    ...
    private void loadAdminAnnonces(List<Annonce> preloaded) {
        try {
            List<Annonce> list = preloaded != null ? preloaded : serviceAnnonce.getAll();
            renderAdminAnnonceCards(list);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    ...
    private void renderAdminAnnonceCards(List<Annonce> annonces) {
        ...
        for (Annonce a : annonces) {
            ...
            HBox row = new HBox(10, lbl, btnDelete);
            annonceAdminContainer.getChildren().add(row);
        }
    }
    @FXML
    ...
    //_________________________Commentaires Chaima_________________________________________________________
    ...
    @FXML
    public void showAdminNotifications(ActionEvent event) {
        List<String> currentLowStock = buildCurrentLowStockMessages();
        if (unreadStockNotifications.isEmpty() && currentLowStock.isEmpty()) {
            alert(Alert.AlertType.INFORMATION, "Aucune notification pour le moment.");
            return;
        }
        ...
        popup.initModality(javafx.stage.Modality.APPLICATION_MODAL);
        popup.setTitle("Notifications admin");
        ...
        Label title = new Label("🔔 Notifications de stock");
        title.setStyle("-fx-font-size:22px; -fx-font-weight:bold; -fx-text-fill:#00b3a6;");
        Label sub = new Label("Surveillez les médicaments en stock faible et réapprovisionnez rapidement.");
        sub.setStyle("-fx-font-size:13px; -fx-text-fill:#6b7280;");

        VBox content = new VBox(10);
        content.setFillWidth(true);

        if (!unreadStockNotifications.isEmpty()) {
            Label sec1 = new Label("Nouvelles notifications");
            sec1.setStyle("-fx-font-size:14px; -fx-font-weight:700; -fx-text-fill:#0f766e;");
            content.getChildren().add(sec1);
            for (String msg : unreadStockNotifications) {
                content.getChildren().add(notificationRow(msg, "#fff7ed", "#fdba74"));
            }
        }

        if (!currentLowStock.isEmpty()) {
            Label sec2 = new Label("Stocks faibles actuels (≤ 5)");
            sec2.setStyle("-fx-font-size:14px; -fx-font-weight:700; -fx-text-fill:#0f766e; -fx-padding:8 0 0 0;");
            content.getChildren().add(sec2);
            for (String msg : currentLowStock) {
                content.getChildren().add(notificationRow(msg, "#fef2f2", "#fca5a5"));
            }
        }
        ...
        VBox.setVgrow(sc, javafx.scene.layout.Priority.ALWAYS);

        Button btnMarkRead = new Button("✅ Tout marquer comme lu");
        btnMarkRead.getStyleClass().add("btn-secondary");
        btnMarkRead.setOnAction(e -> {
            unreadStockNotifications.clear();
            refreshNotificationBadge();
            popup.close();
        });
        ...
        btnClose.setOnAction(e -> popup.close());
        HBox actions = new HBox(10, btnMarkRead, btnClose);
        actions.setAlignment(javafx.geometry.Pos.CENTER_RIGHT);

        root.getChildren().addAll(title, sub, sc, actions);
    ...
    private HBox notificationRow(String text, String bg, String border) {
        ...
        row.setAlignment(javafx.geometry.Pos.TOP_LEFT);
        row.setStyle("-fx-background-color:" + bg + "; -fx-border-color:" + border + "; -fx-border-radius:10; -fx-background-radius:10; -fx-padding:10 12;");
        return row;
    }

    private void evaluateLowStockNotifications() {
        List<Medicament> all = medicamentService.getAll();
        for (Medicament m : all) {
            int id = m.getId();
            int stock = m.getStock();
            int previous = lastKnownMedStock.getOrDefault(id, Integer.MAX_VALUE);
            if (stock <= 5) {
                activeLowStockMedIds.add(id);
                if (previous > 5) {
                    unreadStockNotifications.add("⚠ " + m.getNomMedicament() + " stock faible (" + stock + "). Pensez à réapprovisionner.");
                }
            } else {
                activeLowStockMedIds.remove(id);
            }
            lastKnownMedStock.put(id, stock);
        }
    }

    private List<String> buildCurrentLowStockMessages() {
        List<String> out = new ArrayList<>();
        for (Medicament m : medicamentService.getAll()) {
            if (m.getStock() <= 5) {
                out.add("• " + m.getNomMedicament() + " : stock " + m.getStock());
            }
        }
        return out;
    }
    ...
    private void refreshNotificationBadge() {
        if (adminNotifButton == null) {
            return;
        }
        int unread = unreadStockNotifications.size();
        int lowNow = activeLowStockMedIds.size();
        adminNotifButton.setText("🔔 Notifications (" + unread + ")");
        if (lowNow > 0) {
            adminNotifButton.setTooltip(new Tooltip(lowNow + " médicament(s) en stock faible"));
        } else {
            adminNotifButton.setTooltip(new Tooltip("Aucun stock faible"));
        }
    }

    private void loadClinicalStats() {
        List<Pharmacie> pharmacies = pharmacieService.getAll();
        List<Medicament> medicaments = medicamentService.getAll();

        if (pharmacyHospitalPieChart != null) {
            long withHospital = pharmacies.stream().filter(p -> p.getHopital() != null && !p.getHopital().isBlank()).count();
            long withoutHospital = pharmacies.size() - withHospital;
            pharmacyHospitalPieChart.setData(FXCollections.observableArrayList(
                    new PieChart.Data("Avec hôpital", withHospital),
                    new PieChart.Data("Sans hôpital", withoutHospital)
            ));
        }

        if (medicamentStockPieChart != null) {
            long low = medicaments.stream().filter(m -> m.getStock() <= 5).count();
            long medium = medicaments.stream().filter(m -> m.getStock() > 5 && m.getStock() <= 30).count();
            long high = medicaments.stream().filter(m -> m.getStock() > 30).count();
            medicamentStockPieChart.setData(FXCollections.observableArrayList(
                    new PieChart.Data("Stock faible (≤5)", low),
                    new PieChart.Data("Stock moyen (6-30)", medium),
                    new PieChart.Data("Stock élevé (>30)", high)
            ));
        }

        if (pharmacyTopHospitalsBarChart != null) {
            pharmacyTopHospitalsBarChart.getData().clear();
            LinkedHashMap<String, Integer> byHospital = new LinkedHashMap<>();
            for (Pharmacie p : pharmacies) {
                String h = p.getHopital() == null || p.getHopital().isBlank() ? "Non spécifié" : p.getHopital().trim();
                byHospital.put(h, byHospital.getOrDefault(h, 0) + 1);
            }
            List<java.util.Map.Entry<String, Integer>> top = byHospital.entrySet().stream()
                    .sorted((a, b) -> Integer.compare(b.getValue(), a.getValue()))
                    .limit(6)
                    .toList();
            XYChart.Series<String, Number> s = new XYChart.Series<>();
            s.setName("Pharmacies");
            for (java.util.Map.Entry<String, Integer> e : top) {
                s.getData().add(new XYChart.Data<>(e.getKey(), e.getValue()));
            }
            pharmacyTopHospitalsBarChart.getData().add(s);
        }

        if (medicamentCategoryBarChart != null) {
            medicamentCategoryBarChart.getData().clear();
            LinkedHashMap<String, Integer> byCat = new LinkedHashMap<>();
            for (Medicament m : medicaments) {
                String c = m.getCategorie() == null || m.getCategorie().isBlank() ? "Non classé" : m.getCategorie().trim();
                byCat.put(c, byCat.getOrDefault(c, 0) + 1);
            }
            List<java.util.Map.Entry<String, Integer>> top = byCat.entrySet().stream()
                    .sorted((a, b) -> Integer.compare(b.getValue(), a.getValue()))
                    .limit(8)
                    .toList();
            XYChart.Series<String, Number> s = new XYChart.Series<>();
            s.setName("Médicaments");
            for (java.util.Map.Entry<String, Integer> e : top) {
                s.getData().add(new XYChart.Data<>(e.getKey(), e.getValue()));
            }
            medicamentCategoryBarChart.getData().add(s);
        }

        if (medicamentExpiryLineChart != null) {
            medicamentExpiryLineChart.getData().clear();
            XYChart.Series<String, Number> s = new XYChart.Series<>();
            s.setName("Expirations");
            List<YearMonth> months = new ArrayList<>();
            YearMonth now = YearMonth.now();
            for (int i = 0; i < 12; i++) {
                months.add(now.plusMonths(i));
            }
            DateTimeFormatter f = DateTimeFormatter.ofPattern("MM/yy");
            for (YearMonth ym : months) {
                int count = (int) medicaments.stream()
                        .filter(m -> m.getDateExpiration() != null && YearMonth.from(m.getDateExpiration().toLocalDate()).equals(ym))
                        .count();
                s.getData().add(new XYChart.Data<>(ym.format(f), count));
            }
    ...
    /**
     * Analyse IA (Hugging Face) : actifs vs risque de désengagement à partir de {@code login_count} et {@code last_login_at}.
     */
    @FXML
    public void runUserEngagementInsightAnalysis(ActionEvent event) {
        Node src = (Node) event.getSource();
        src.setDisable(true);
        Task<String> task = new Task<>() {
            @Override
            protected String call() {
                return engagementInsightService.analyzeEngagementFromUsers(userService.getAll());
            }
        };
        task.setOnSucceeded(e -> {
            src.setDisable(false);
            showEngagementInsightWindow(task.getValue());
        });
        task.setOnFailed(e -> {
            src.setDisable(false);
            Throwable ex = task.getException();
            alert(Alert.AlertType.ERROR, ex != null ? ex.getMessage() : "Erreur lors de l'analyse d'engagement.");
        });
        Thread t = new Thread(task, "hf-user-engagement");
        t.setDaemon(true);
        t.start();
    }

    private void showEngagementInsightWindow(String report) {
        Stage popup = new Stage();
        popup.setTitle("Analyse d'engagement — DHC");
        popup.setMinWidth(640);
        popup.setMinHeight(480);

        String body = report != null ? report : "";
        boolean localFallback = body.startsWith("⚠️") || body.contains("— Rapport local");
        List<User> allUsers = userService.getAll();
        String htmlDoc = EngagementReportHtmlFormatter.toFullHtmlDocument(body, allUsers);

        Label badge = new Label(localFallback ? "Mode secours (local)" : "Hugging Face · IA");
        badge.setStyle("-fx-background-color:rgba(255,255,255,0.22); -fx-text-fill:white; -fx-padding:6 14; "
                + "-fx-background-radius:20; -fx-font-size:11px; -fx-font-weight:bold;");

        Label title = new Label("Engagement utilisateurs");
        title.setStyle("-fx-font-size:22px; -fx-font-weight:bold; -fx-text-fill:white;");

        Label subtitle = new Label("Synthèse et recommandations pour l'équipe admin (connexions & dernière activité).");
        subtitle.setWrapText(true);
        subtitle.setStyle("-fx-font-size:13px; -fx-text-fill:rgba(255,255,255,0.92); -fx-max-width:520;");

        VBox titleCol = new VBox(8, title, subtitle);
        Region headerSpacer = new Region();
        HBox.setHgrow(headerSpacer, Priority.ALWAYS);
        HBox header = new HBox(18, titleCol, headerSpacer, badge);
        header.setAlignment(Pos.CENTER_LEFT);
        header.setPadding(new Insets(20, 24, 20, 24));
        header.setStyle("-fx-background-color: linear-gradient(to right, #00b3a6, #00a89c, #0d6b7a);");

        WebView reportWeb = new WebView();
        reportWeb.setMinHeight(420);
        reportWeb.setPrefHeight(520);
        reportWeb.getEngine().loadContent(htmlDoc, "text/html");
        VBox.setVgrow(reportWeb, Priority.ALWAYS);

        ScrollPane scroll = new ScrollPane(reportWeb);
        scroll.setFitToWidth(true);
        scroll.setFitToHeight(true);
        scroll.setStyle("-fx-background:transparent; -fx-background-color:transparent;");
        VBox.setVgrow(scroll, Priority.ALWAYS);

        VBox center = new VBox(0, scroll);
        center.setPadding(new Insets(12, 20, 10, 20));
        center.setStyle("-fx-background-color:#eef2f6;");

        Button close = new Button("Fermer");
        close.setPrefWidth(128);
        ...
        try {
            var themeUrl = getClass().getResource("/theme.css");
            if (themeUrl != null) {
        ...
        if (userCardsContainer != null && userCardsContainer.getScene() != null) {
            popup.initOwner(userCardsContainer.getScene().getWindow());
        }
        popup.setScene(scene);
        popup.show();
    }
    ...
    /** Onglet Statistiques : événements + engagement utilisateurs (scroll dans le FXML). */
    private void loadStatisticsTab() {
        if (eventStatsSummaryLabel == null && eventThemePieChart == null
                && eventMonthBarChart == null && eventLieuBarChart == null
                && userEngagementPie == null) {
            return;
        }
        EventStatistics st = eventService.computeStatistics();
        int totalSql = eventService.countEventsSql();
        long sumPartSql = eventService.sumParticipantsSql();
        Map<String, Long> themeSql = eventService.countByThemeSql();

        if (eventStatsSummaryLabel != null) {
            eventStatsSummaryLabel.setText(String.format(Locale.FRENCH,
                    "Total : %d événement(s) (vérification SQL : %d) · à venir : %d · passés : %d · aujourd’hui : %d · "
                            + "participants cumulés : %d (SQL) · moyenne : %.1f participant(s) / événement",
                    st.getTotalEvents(),
                    totalSql,
                    st.getUpcomingCount(),
                    st.getPastCount(),
                    st.getTodayCount(),
                    sumPartSql,
                    st.getAverageParticipants()));
        }

        if (eventThemePieChart != null) {
            eventThemePieChart.setData(FXCollections.observableArrayList(
                    themeSql.entrySet().stream()
        ...
        if (eventMonthBarChart != null) {
            eventMonthBarChart.getData().clear();
            XYChart.Series<String, Number> monthSeries = new XYChart.Series<>();
            monthSeries.setName("Événements");
            DateTimeFormatter monthFmt = DateTimeFormatter.ofPattern("MMM yyyy", Locale.FRENCH);
            for (Map.Entry<String, Long> e : st.getCountByMonth().entrySet()) {
                YearMonth ym = YearMonth.parse(e.getKey());
                monthSeries.getData().add(new XYChart.Data<>(ym.format(monthFmt), e.getValue()));
            }
            eventMonthBarChart.getData().add(monthSeries);
        }

        if (eventLieuBarChart != null) {
            eventLieuBarChart.getData().clear();
            XYChart.Series<String, Number> lieuSeries = new XYChart.Series<>();
            lieuSeries.setName("Par lieu");
            List<Map.Entry<Integer, Long>> topLieux = st.getCountByLieuId().entrySet().stream()
                    .sorted((a, b) -> Long.compare(b.getValue(), a.getValue()))
                    .limit(8)
                    .toList();
            for (Map.Entry<Integer, Long> e : topLieux) {
                Lieu l = lieuService.getOneById(e.getKey());
                String label = l != null && l.getNomLieu() != null && !l.getNomLieu().isBlank()
                        ? l.getNomLieu()
                        : ("Lieu #" + e.getKey());
                lieuSeries.getData().add(new XYChart.Data<>(label, e.getValue()));
            }
            eventLieuBarChart.getData().add(lieuSeries);
        }

        if (userEngagementPie != null) {
            UserEngagementCharts.populate(
                    userService.getAll(),
                    userEngagementPie,
                    userRecencyBarChart,
                    userAvgLoginsLineChart,
                    userLoginScatterChart);
        }
    }
                ...
                eventService.getAll().stream()
                        .filter(e ->
                                String.valueOf(e.getId()).contains(search) ||
                                        (e.getTitreEvent() != null && e.getTitreEvent().toLowerCase().contains(search)) ||
                                        (e.getThemeSante() != null && e.getThemeSante().toLowerCase().contains(search)) ||
                                        (e.getDescription() != null && e.getDescription().toLowerCase().contains(search)) ||
                                        (e.getDateEvent() != null && e.getDateEvent().toString().contains(search)) ||
                                        (e.getHeureDebut() != null && e.getHeureDebut().toString().contains(search)) ||
                                        (e.getHeureFin() != null && e.getHeureFin().toString().contains(search)) ||
                                        (e.getNbParticipant() != null && String.valueOf(e.getNbParticipant()).contains(search)) ||
                                        String.valueOf(e.getUserId()).contains(search) ||
                                        String.valueOf(e.getEventLieuId()).contains(search)
                        )
                        .collect(Collectors.toList())
        );
